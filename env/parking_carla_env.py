...
from config import CarlaEnvConfig
import time


class CarlaEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    def __init__(
        self,
        env_cfg=CarlaEnvConfig(),
    ) -> None:
        super().__init__()
    ...
    def reset(
        ...
        *,
        ...
        Set ego car.
        """
        self.ego = carla_utils.spawn_parking_ego_car(self.sim_world)
        ...
        ) = carla_utils.set_parking_obstacles(
            self.sim_world, obstacle_num=self.env_cfg.obstacle_num
        )
        """
        Set cameras.
        """
        ...
        return (
            self.local_se2state,
            {
                "local_obstacle_polygon_list": [],
                "local_parking_polygon_list": [],
    ...
    def step(
        ...
        return (
            ...
            {
                "local_obstacle_polygon_list": perception_obstacle_polygon_list,
                "local_parking_polygon_list": perception_parking_polygon_list,
                "global_obstacle_polygon_list": global_obstacle_polygon_list,
                "global_parking_polygon_list": global_parking_polygon_list,
            },
        )
    ...
    def perception(self):
        ...
        # return self.perception_todo(self.bev.data)

        """
        Generate obstacle vertexes list and parking vertexes list from carla server.
        """

        (
            obstacle_vertexes_list_in_carlamap,
            parking_vertexes_list_in_carlamap,
        ) = carla_utils.generate_perception_from_cheating(
            self.obstacle_actor_list, self.target_transform_list
        )
        ...
        for obstacle_vertexes in obstacle_vertexes_list_in_carlamap:
            obstacle_polygon_list_in_global_se2 += [Polygon(obstacle_vertexes)]

        for parking_vertexes in parking_vertexes_list_in_carlamap:
            parking_polygon_list_in_global_se2 += [Polygon(parking_vertexes)]

        obstacle_vertexes_list_in_current_se2 = ct.change_vertexes_list_coord(
            obstacle_vertexes_list_in_carlamap, self.global_se2state.se2
        )

        for obstacle_vertexes in obstacle_vertexes_list_in_current_se2:
            obstacle_polygon_list_in_current_se2 += [Polygon(obstacle_vertexes)]

        parking_vertexes_list_in_current_se2 = ct.change_vertexes_list_coord(
            parking_vertexes_list_in_carlamap, self.global_se2state.se2
        )

        for parking_vertexes in parking_vertexes_list_in_current_se2:
